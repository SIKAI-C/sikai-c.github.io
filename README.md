...
- Optional lightweight posts section with dedicated layout and `/posts/` landing page
...
├── _layouts                  
|   ├── base.html                             #  shared shell for every page
...
├── _posts                                    #  Markdown files that become individual posts
├── assets                                    #  some files
├── html_source_file                          #  compiled HTML files
├── posts                                     #  the /posts/ landing page
...
If you enable the optional posts section, add a short blurb linking to `/posts/` so visitors can discover the stream of mini updates.
...
To publish lightweight posts, copy `_posts/_TEMPLATE.md`, rename it with the `YYYY-MM-DD-your-title.md` pattern, and place it in the `_posts` folder—the entry will appear automatically on `/posts/`.
...
If you'd like to change the theme's HTML layout, you may edit `_layout/homepage.html`. The posts workflow uses `_layouts/post.html` plus the `/posts/` index, so that is the place to tweak how the post cards and navigation tabs look.
